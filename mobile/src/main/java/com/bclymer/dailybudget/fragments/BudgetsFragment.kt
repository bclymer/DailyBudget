...
class BudgetsFragment() : BaseFragment() {
    ...
    private var mBudgetList: MutableList<Budget>? = null

    private var mCallback: BudgetSelectedCallback? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        for (budget in mBudgetList!!) {
            budget.updateCache()
        }

        val budgetAdapter = BudgetAdapter()
        mAdapter = ScaleInAnimationAdapter(budgetAdapter)
    ...
    private inner class BudgetAdapter : BaseAdapter() {
        ...
        override fun getCount(): Int {
            return if (mBudgetList != null) mBudgetList!!.size else 0
        }

        override fun getItem(position: Int): Budget {
            return mBudgetList!![position]
        }
